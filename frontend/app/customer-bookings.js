...
export default function CustomerBookingsPage({ userId = 1 }) {
  ...
  // Debug logging
  console.log('👤 CustomerBookingsPage - userId:', userId);
  ...
  const loadBookings = async () => {
    setLoading(true);
    try {
      const url = `${apiUrl}/api/bookings?role=customer&user_id=${userId}`;
      console.log('👤 Customer API Call:', url);
      ...
      console.log('👤 Customer ID filter check - looking for customer_id:', userId);
      
      if (response.ok) {
        // Verify each booking belongs to this customer
        const filteredBookings = Array.isArray(data) ? data.filter(booking => {
          console.log(`👤 Booking ${booking.id}: customer_id=${booking.customer_id}, expected=${userId}`);
          return booking.customer_id === parseInt(userId);
        }) : [];
        
        console.log('👤 Filtered bookings for customer:', filteredBookings.length);
    ...
    >
      ...
      <View style={styles.bookingInfo}>
        <Text style={styles.chefName}>
          Chef: {item.chef_first_name} {item.chef_last_name}
        </Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>My Customer Bookings</Text>
        <Text style={styles.userInfoText}>
          Your Customer Bookings (ID: {userId})
        </Text>
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              Book a chef to see your reservations here
            </Text>
          </View>
        }
            ...
            <ScrollView style={styles.modalContent}>
              ...
              <View style={styles.detailSection}>
                ...
                <Text style={styles.detailValue}>
                  {selectedBooking.chef_first_name} {selectedBooking.chef_last_name}
                </Text>
              </View>
              ...
              <View style={styles.actionSection}>
                {canCancel(selectedBooking) && (
                  <>
                    <Text style={styles.actionLabel}>Cancellation Note (Optional):</Text>
                    <TextInput
