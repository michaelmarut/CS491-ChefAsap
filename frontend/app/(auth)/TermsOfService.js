...
export default function TermsOfService() {
  ...
  const fetchTerms = async () => {
    try {
      const response = await fetch(`${apiUrl}/agreement/active?user_type=all`);
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-base-100 dark:bg-base-dark-100 items-center justify-center">
        <ActivityIndicator size="large" color="#FF6B35" />
        <Text className="mt-4 text-primary-400 dark:text-dark-400">Loading Terms of Service...</Text>
      </View>
    );
  }
  ...
  if (error) {
    return (
      <ScrollView className="flex-1 bg-base-100 dark:bg-base-dark-100 p-5">
        <Text className="text-3xl font-bold text-center text-primary-500 dark:text-dark-500 mb-4">
          Terms of Service
        </Text>
        <Card>
          <Text className="text-center text-warm-gray mb-4">{error}</Text>
  ...
  return (
    <ScrollView className="flex-1 bg-base-100 dark:bg-base-dark-100 p-5">
      ...
      <Card>
        <Text className="text-sm text-primary-400 dark:text-dark-400 leading-6 whitespace-pre-line">
          {agreement?.content || getDefaultTerms()}
        </Text>
      </Card>
...
// Fallback terms in case server is unavailable
function getDefaultTerms() {
  return `CHEFASAP TERMS OF SERVICE
